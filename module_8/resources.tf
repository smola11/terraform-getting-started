...
# It will create a text string based on a template we give it.
data "template_file" "public_cidrsubnet" {

  # We want x number of strings. So it will create 2 subnet strings (for Development env).
...
# NETWORKING #
# This module exists on Terraform registry. Terraform will pull down this module from registry.
module "vpc" {
...
resource "aws_security_group" "elb-sg" {
  ...
  # We refere to the vpc_id generated by the module.
...
resource "aws_instance" "nginx" {
  ...
  # Assure that bucket and IAM role is fully complete before creating EC2 instances.
